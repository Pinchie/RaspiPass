...
then
	...
	# Update 0.7.4+ - add /var/raspipass and /var/log/apache2 RAMdisk entries to fstab, if they do not exist
	echo
	...
	then
		...
		echo "tmpfs    /var/raspipass    tmpfs    defaults,noatime,nosuid,mode=0777,size=10m    0 0" >> /etc/fstab
		reboot_required=1
	fi
